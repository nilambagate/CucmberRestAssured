package com.Basics.deleteRequestSD;
...
public class MyDeleteRequest {
	...
	public void user_hit_the_post_request_with_String() throws IOException{
		...
		Response postresp = RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().
		...
		//Delete user
		resp=RestAssured.given().accept(ContentType.JSON).relaxedHTTPSValidation().when().delete("https://reqres.in/api/users/"+userID);	
		resp.then().log().all();
